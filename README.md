...
**Live:** https://techshield-tech.github.io/bcrypt-generator/

Part of [MMOALL Developer Tools](https://mmoall.com/tools).
Also available at [mmoall.com/tools/bcrypt-generator](https://mmoall.com/tools/bcrypt-generator).
...
├── index.css             # Tailwind import + tool-kit theme tokens
...
primitives, SEO) is no longer local to this repo — it comes from the
[`@mmoall/tool-kit`](https://github.com/techshield-tech/tool-kit) npm
dependency instead.
...
```bash
git clone https://github.com/techshield-tech/bcrypt-generator.git
cd bcrypt-generator
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/bcrypt-generator/**
(note the `/bcrypt-generator/` path, see [Base path](#base-path)).
...
| otherwise (default)     | `/bcrypt-generator/` | GitHub Pages                |
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/bcrypt-generator` repository.
...
`bun run build` with the default `/bcrypt-generator/` base, and publishes `dist/`.
...
<iframe
  id="bcrypt-generator"
  src="https://techshield-tech.github.io/bcrypt-generator/?embed=1&theme=dark"
  style="width: 100%; border: 0;"
  title="JSON Formatter"
></iframe>

<script>
  const iframe = document.getElementById('bcrypt-generator');

  window.addEventListener('message', (event) => {
    const data = event.data;
    if (data?.slug !== 'bcrypt-generator') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'bcrypt-generator' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'bcrypt-generator', height }` | Posted whenever the document height changes (embed mode only) |
